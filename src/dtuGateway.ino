...
void loop()
{
  ...
  if (currentMillis - previousMillisShort >= intervalShort)
  {
    ...
    if (client.connected())
    {
      ...
      {
        writeReqCommand(&client, localTimeSecond);
        Serial.print("\nsetted new power limit from " + String(globalData.powerLimit) + " to " + String(globalData.powerLimitSet) + "\n");
        writeReqRealDataNew(&client, localTimeSecond);
        writeReqGetConfig(&client, localTimeSecond); // get approval of setting new value
  ...
  if (currentMillis - previousMillisMid >= intervalMid)
  {
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
      ...
      if (!client.connected() && !dtuConnection.dtuActiveOffToCloudUpdate)
      {
      ...
      if (client.connected())
      {
        ...
        writeReqGetConfig(&client, localTimeSecond);
        // client.stop();
